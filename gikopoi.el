...
(defcustom akai-time "[%Y-%m-%d %H:%M:%S] "
  "A `format-time-string` for akai."
  :group 'gikopoi
  :type 'string)
    ...
    (error
     (let ((msg (error-message-string err)))
       (unless (string-match-p "No usable sound device driver found" msg)
         (message "Unhandled websocket error: %s" msg)))))) ; No drivers in WSL
...
(defvar-local gikopoi--last-point-max 0) ; track cursor
...
;; Def: scroll keys on the message buffer keymap
(define-key gikopoi-msg-mode-map (kbd "<next>") #'gikopoi-scroll-down-safely)
...
(defun gikopoi--pad-chat-cleanup-space ()
  ...
  (let* ((win (get-buffer-window gikopoi-message-buffer))
         ...
         (visible-lines (window-body-height win))
         (padding-lines (max 0 (- visible-lines line-count))))
    (save-excursion
      (goto-char (point-min))
...
;;         (gikopoi-send-blank)  ;; send blank if no input
;;       (cond
...
;;         (message "[SENT!]"))))
;;     ;; Focus back to Gikopoi buffer after sending
...
(defvar gikopoi-auto-ignore-file
  (expand-file-name "./auto-ignore.txt")
  "File storing the list of automatically ignored users, kept in the current directory.")

(defun gikopoi--load-auto-ignore-list ()
  "Read ignored users from `gikopoi-auto-ignore-file` and return as a list."
...
(defun gikopoi--save-auto-ignore-list (names)
  "Write NAMES list to `gikopoi-auto-ignore-file`."
  (with-temp-file gikopoi-auto-ignore-file
    (dolist (name (delete-dups names))
      (insert name "\n"))))

(defun gikopoi--ensure-auto-ignore-file ()
  "Ensure `auto-ignore.txt` exists in the current directory."
...
If NAME isn’t online, it’s still saved so they’re ignored next time they appear.
This version is case-sensitive."
  (interactive "sAuto-ignore username: ")
  (gikopoi--ensure-auto-ignore-file)
  (let* ((names (gikopoi--load-auto-ignore-list))  ;; preserve original case
    ...
    (if already
        (progn
          ...
          (message "%s removed from auto-ignore list" name))
      (push name names)
      (when user (setf (gikopoi-user-ignored-p user) t))
      (message "%s added to auto-ignore list" name))
...
;;          (already (member-ignore-case name names))
;;          (user (gikopoi-user-by-name name)))
;;     (if already
;;         (progn
;;           (setq names (delete name names))
...
;;   "Automatically ignore all users listed in `auto-ignore.txt`."
;;   (interactive)
;;   (gikopoi--ensure-auto-ignore-file)
;;   (let ((names (gikopoi--load-auto-ignore-list)))
;;     (dolist (name names)
...
;;     (message "Loaded %d auto-ignored users" (length names))))
...
(defun gikopoi-load-auto-ignored-users ()
  ...
  (when gikopoi-current-room
    (dolist (user (gikopoi-room-users gikopoi-current-room))
      (when (member (downcase (gikopoi-user-name user))
                    gikopoi-auto-ignore-names)
        (setf (gikopoi-user-ignored-p user) t))))
  (message "Loaded %d auto-ignored users" (length gikopoi-auto-ignore-names)))


;; Automatically load ignores when connecting
;; (add-hook 'gikopoi-connected-hook #'gikopoi-load-auto-ignored-users)
...
(cl-defmethod (setf gikopoi-user-name) (name (user gikopoi-user))
  (let ((name (if (string-empty-p name)
		  ;(or (alist-get 'default_user_name gikopoi-lang-alist)
		      "Spy";) ; commented out Anonymous names
		name)))
...
(cl-defmethod shared-initialize :after ((this gikopoi-user) initargs)
  (let* ((h (/ (mod (sxhash (slot-value this 'id)) 360.0) 360.0)) ; hue 0–1
         (s 0.6)   ; medium saturation
         (l 0.7)   ; lighter lightness for Wombat
         (rgb (color-hsl-to-rgb h s l))
         (color (apply #'color-rgb-to-hex rgb)))
    (setf (slot-value this 'name-color) color
          (gikopoi-user-name this) (slot-value this 'name)))
  (when (equal gikopoi-current-user-id
	       (slot-value this 'id))
    (setq gikopoi-current-user this)))
...
(defun gikopoi--log-to-file (text)
  "Append TEXT to a log file named by today's date in `gikopoi-log-directory`.
   Does nothing if `gikopoi-logger` is nil."
  (when gikopoi-logger
    ...
    (let* ((name (gikopoi-user-name user))
           ...
           (formatted-lines
            (mapcar (lambda (line)
                      (format "%s: %s\n" name line))
                    lines)))
      (gikopoi-user-insert-message user (string-join formatted-lines "")))))
...
(cl-defmethod gikopoi-user-roll-die ((user gikopoi-user) base sum times)
  (gikopoi-user-insert-message user (format "[DICE] * %s rolled %s x d%s and got %s!\n"
...
(defvar gikopoi-auto-ignore-applied-p nil
  "Non-nil if auto-ignore has been applied to the current room.")

(gikopoi-defevent server-update-current-room-state ((currentRoom connectedUsers streams))
  "Update current room and auto-ignore users."
...
;; (gikopoi-defevent server-update-current-room-state ((currentRoom connectedUsers streams))
;;   "Update the current room state, mark ignored users, and display messages only from non-ignored users."
;;   ;; Mark room as loading
;;   (setq gikopoi-current-room-loading-p t)
...
;;                              :users connectedUsers)))
;;     (setq gikopoi-current-room room)
;;     ;; Reset the auto-ignore flag for new room
;;     (setq gikopoi-auto-ignore-applied-p nil))

;;   ;; Apply auto-ignore only once
;;   (unless gikopoi-auto-ignore-applied-p
;;     (when (fboundp 'gikopoi-load-auto-ignored-users)
;;       (gikopoi-load-auto-ignored-users)
;;       (setq gikopoi-auto-ignore-applied-p t)))
...
(defvar gikopoi--last-args nil
  "Last arguments used to connect to gikopoi.")

(defun gikopoi-reconnect ()
  "Quit and reconnect to gikopoi using last known arguments."
...
(defcustom gikopoi-auto-start-reconnect-timer nil
  "If non-nil, start the reconnect timer automatically on initialization."
  :type 'boolean
  :group 'gikopoi)

(defun gikopoi-maybe-start-reconnect-timer (&rest _args)
  "Start the reconnect timer if `gikopoi-auto-start-reconnect-timer' is non-nil."
...
(defun gikopoi-start-reconnect-timer ()
  (interactive)
  (let* ((minutes gikopoi-reconnect-timer-minutes)
         (seconds (* minutes 60))
         (hrs (/ minutes 60))
         (mins (% minutes 60)))
    ...
    (message "gikopoi: reconnect timer started (%d hr %d min)"
             hrs mins)))
...
;;     (cancel-timer gikopoi-reconnect-timer))
;;   (setq gikopoi-reconnect-timer
;;         (run-at-time "12 hours" (* 12 60 60) #'gikopoi-reconnect))
;;   (message "gikopoi: reconnect timer started"))
...
(defun gikopoi-quit-silent ()
  "Quit gikopoi without confirmation."
  (run-hooks 'gikopoi-quit-functions))
